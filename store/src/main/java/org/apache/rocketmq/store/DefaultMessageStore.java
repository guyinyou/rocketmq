...
public class DefaultMessageStore implements MessageStore {
    ...
    public void doReput() {
        this.reputMessageService.doReput();
    }

    class CreateDispatchRequestTask {
        ...
        public CreateDispatchRequestTask(ByteBuffer byteBuffer, int pos, int size, int id) {
    ...
    class OrderlyQueue {
        ...
        public OrderlyQueue(int bufferNum){
    ...
    class ReputMessageService extends ServiceThread {
        ...
        private int reputThreadPoolNums = 4;

        private final ConcurrentLinkedQueue<CreateDispatchRequestTask> createDispatchRequestTaskQueue = new ConcurrentLinkedQueue<>();
        private final AtomicInteger noOverCount = new AtomicInteger(0);

        private final OrderlyQueue orderlyQueue;
        ...
        private int idCnt = 0;
        public void createDispatchRequest(ByteBuffer byteBuffer, int pos, int size) {
            if (pos < 0) {
                return;
            }
            CreateDispatchRequestTask task = new CreateDispatchRequestTask(byteBuffer, pos, size, idCnt++);
            this.noOverCount.getAndIncrement();
            createDispatchRequestTaskQueue.offer(task);
        }

        public void doCreateDispatchRequest(List<DispatchRequest> cacheList, CreateDispatchRequestTask task) {
            ByteBuffer tmpByteBuffer = task.byteBuffer.duplicate();
            tmpByteBuffer.position(task.pos);
            tmpByteBuffer.limit(task.pos + task.size);
            while (tmpByteBuffer.hasRemaining()) {
                ...
                cacheList.add(dispatchRequest);
            }
            orderlyQueue.put(task.id, cacheList.toArray(new DispatchRequest[cacheList.size()]));
        }

        public ReputMessageService() {
            this.orderlyQueue = new OrderlyQueue(16);
            Thread[] threads = new Thread[reputThreadPoolNums];
            for (int i = 0; i < threads.length; i++) {
                threads[i] = new Thread(new Runnable() {
                    @Override
                    public void run() {
                        List<DispatchRequest> cacheList = new ArrayList<>();
                        while (true) {
                            CreateDispatchRequestTask task = createDispatchRequestTaskQueue.poll();
                            if (task != null) {
                                cacheList.clear();
                                doCreateDispatchRequest(cacheList, task);
                                noOverCount.getAndDecrement();
                            } else {
                                try {
                                    Thread.sleep(1);
                                } catch (InterruptedException e) {
            ...
            new Thread(()->{
                List<DispatchRequest[]> cacheList = new ArrayList<>();
                while (true){
                    orderlyQueue.get(cacheList);
                    if (!cacheList.isEmpty()){
                        for (DispatchRequest[] dispatchRequests : cacheList) {
                ...
                if (result != null) {
                    ...
                    try {
                        ...
                        for (int readSize = 0; readSize < result.getSize() && reputFromOffset < DefaultMessageStore.this.getConfirmOffset() && doNext; ) {
                            ...
                            if (size > 0) {
                                if (readyPos == -1) {
                                    readyPos = byteBuffer.position() - 8;
                                    readySize = 0;
                                }
                                readySize += size;
                                if (readySize > 1024 * 1024 * 4) {
                                    this.createDispatchRequest(byteBuffer, readyPos, readySize);
                                    readyPos = -1;
                                    readySize = -1;
                                }
                        ...
                        this.createDispatchRequest(result.getByteBuffer(), readyPos, readySize);
                        boolean over = this.noOverCount.get() == 0;
                        while (!over) {
                            try {
                                Thread.sleep(1);
                            } catch (Exception e) {
...
//            System.out.println("ccc: " + this.reputFromOffset);
//            boolean over;
//            do{
//                over = orderlyQueue.isEmpty();
//                try {
//                    Thread.sleep(1);
//                }catch (Exception e){
//                    e.printStackTrace();
